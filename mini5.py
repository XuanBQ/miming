...
def tableExist(name):
    ...
    # todo:
	sql = " "
	dbresult = cur.execute(sql,[tableName])
    #ret = cur.fetchmany(dbresult)
	conn.commit()
	cur.close()
...
# dt: datetime type
def time2TableName(dt):
    return "message_" + datetimeToString(dt)

def getDayTableName(dt):
    name =  time2TableName(dt)
    if tableExist(name) == True:
        createTable(name)
    return name
...
class Upload(tornado.web.RequestHandler):
	def get(self):
		...
		dtStr = self.get_arguments("datetime")
		username = self.get_argument("username").encode('utf-8')
		print qq_id[0]
		print picurl[0]
		print location[0]
		dt = stringToDateTime(dtStr[0])
		tableName = getDayTableName(dt)
		sql="INSERT INTO %s VALUES (%s,%s,%s,%s,%s,%s,0,%s,%s);"
		cur.execute(sql,[tableName,qq_id[0],picurl[0],lat[0],lng[0],location,dt,text,username])
		conn.commit()
		cur.close()
		conn.close()
...
class GetLocalInfo(tornado.web.RequestHandler):
	def get(self):
		...
		lng = self.get_arguments("lng")
		cur = conn.cursor()
		sql = "select * from %s where inDistance(lat,lng,%s,%s,5000)=1 order by datetime limit 8;"
		dbresult = cur.execute(sql,[tableName, lat[0],lng[0]])
		list = []
		res = {}
